...
public class FolderAdapter extends BaseAdapter {
			...
			if (i == 0) {
				...
				if (mFolders.size() > 0) {
					...
					Glide.with((Activity) mContext)
							...
							.override(mImageSize, mImageSize).centerCrop()
							.into(holder.cover);
				}
			} else {
	...
	class ViewHolder {
		...
		void bindData(Folder data) {
			...
			// 显示图片
			Glide.with((Activity) mContext).load(new File(data.cover.path))
					.placeholder(R.drawable.default_error)
					.override(mImageSize, mImageSize).centerCrop().into(cover);
			// TODO 选择标识
		}
	}
