...
public class ImageUtils {
	...
	public static Bitmap toRoundCornerScaleDst(Context context, Bitmap src,
			NinePatchDrawable dstDrawable) {
	...
	public static byte[] makeThumbnail(byte[] data) {
		...
		byte[] thumbnailData = ImageUtils.getResizedImageData(data, 200, 200,
				200, 200, 10240);
	...
	public static String getImagePath(String remoteUrl) {
		String imageName = remoteUrl.substring(remoteUrl.lastIndexOf("/") + 1,
				remoteUrl.length());
		String path = FileUtil.createAppCacheDir().getAbsolutePath() + "/"
				+ imageName;
