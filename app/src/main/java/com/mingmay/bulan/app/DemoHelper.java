...
import com.mingmay.bulan.model.Msg;
...
public class DemoHelper {
					...
					public String getDisplayedText(EMMessage message) {
						...
						if (user != null) {
							if (EaseAtMessageHelper.get().isAtMeMsg(message)) {
								return String.format(appContext
										.getString(R.string.at_your_in_group),
										user.getNick());
							}
	...
	// Intent(Constant.ACTION_GROUP_CHANAGED));
	// }
	//
	// @Override
	// public void onInvitationAccpted(String groupId, String invitee, String
	// reason) {
	//
	// new InviteMessgeDao(appContext).deleteMessage(groupId);
	//
	...
	// Log.d(TAG, invitee + "Accept to join the group：" + _group == null ?
	...
	// msg.setReason(reason);
	// msg.setGroupInviter(invitee);
	// Log.d(TAG, invitee + "Declined to join the group：" + group == null ?
	...
	//
	// @Override
	// public void onAutoAcceptInvitationFromGroup(String groupId, String
	// inviter, String inviteMessage) {
	// // got an invitation
	// String st3 =
	...
	private EaseUser getUserInfo(String username) {
		...
		if (sysUser != null) {
			user.setNick(sysUser.firstName);
			user.setAvatar(sysUser.userImg);
		}
		return user;
	}
			...
			public void onMessageReceived(List<EMMessage> messages) {
				for (EMMessage message : messages) {
					...
					if (message.getChatType() == ChatType.GroupChat) {
						if (SettingManager.getInstance().isDisNoice(
								message.getTo())) {
							continue;
						}
					}
			...
			public void onCmdMessageReceived(List<EMMessage> messages) {
				for (EMMessage message : messages) {
					...
					EMCmdMessageBody cmdMsgBody = (EMCmdMessageBody) message
							.getBody();
					final String action = cmdMsgBody.action();// get your
																// predefined
																// action
					EMLog.d(TAG, String.format("Command：action:%s,message:%s",
	...
	// public void asyncFetchContactsFromServer(final
